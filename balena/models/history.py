...
class HistoryBaseClass(ABC):
    ...
    def _history_timerange_filter_with_guard(self, fromDate=None, toDate=None):
        ...
        if fromDate is not None:
            if isinstance(fromDate, datetime):
                rawTimeRangeFilters.append(
                    f"(created_at ge datetime'{fromDate.isoformat()}')"
                )
            else:
        ...
        if toDate is not None:
            if isinstance(toDate, datetime):
                rawTimeRangeFilters.append(
                    f"(created_at le datetime'{toDate.isoformat()}')"
                )
            else:
    ...
    def _get_history_by_resource_and_filter(
        self, resource_name, fromDate=None, toDate=None, **options
    ):

        rawTimeRangeFilter = self._history_timerange_filter_with_guard(
...
class DeviceHistory(HistoryBaseClass):
    ...
    def get_all_by_device(
        self, uuid_or_id, fromDate=datetime.utcnow() + timedelta(days=-7), toDate=None
    ):
        """
        ...
        if is_id(uuid_or_id):
            ...
            return self._get_history_by_resource_and_filter(
                "device", uuid=uuid_or_id, fromDate=fromDate, toDate=toDate
            )
        else:
            raise exceptions.InvalidParameter("uuid_or_id", uuid_or_id)

    def get_all_by_application(
        self, app_id, fromDate=datetime.utcnow() + timedelta(days=-7), toDate=None
    ):
        """
