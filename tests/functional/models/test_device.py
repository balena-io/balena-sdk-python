...
class TestDevice(unittest.TestCase):
    ...
    def test_04_register(self):
        ...
        with self.assertRaises(self.helper.balena_exceptions.InvalidDeviceType):
            self.balena.models.device.register(type(self).app['id'], self.balena.models.device.generate_uuid(), 'foobarbaz')

        # should be rejected when providing a device type incompatible with the application.
        with self.assertRaises(self.helper.balena_exceptions.InvalidDeviceType):
            self.balena.models.device.register(type(self).app['id'], self.balena.models.device.generate_uuid(), 'intel-nuc')
    ...
    def test_09_rename(self):
        ...
        self.assertEqual(
            self.balena.models.device.get_name(type(self).device['uuid']),
            'test-device'
        )
    ...
    def test_12_get_status(self):
        ...
        self.assertEqual(
            self.balena.models.device.get_status(type(self).device['uuid']),
            'Inactive'
        )
    ...
    def test_22_grant_support_access(self):
        ...
        support_date = datetime.strptime(self.balena.models.device.get(type(self).device['uuid'])['is_accessible_by_support_until__date'], '%Y-%m-%dT%H:%M:%S.%fZ')
    ...
    def test_23_revoke_support_access(self):
        ...
        self.assertIsNone(self.balena.models.device.get(type(self).device['uuid'])['is_accessible_by_support_until__date'])
    ...
    def test_30_get_supervisor_target_state(self):
        ...
        self.assertEqual(int(list(supervisor_target_state['local']['apps'].keys())[0]), type(self).app['id'])
        self.assertEqual(supervisor_target_state['local']['apps']['{}'.format(type(self).app['id'])]['name'], type(self).app['app_name'])
    ...
    def test_31_remove(self):
        ...
        self.assertEqual(len(self.balena.models.device.get_all_by_application_id(type(self).app['id'])), app_devices_len - 1)
